...
vals.Nh = vals.n*vals.N + vals.m*(vals.N-1) + vals.N-1;
...
Rf = [0,0,0];
for i = 1:vals.N
    vals.x0(x_ind{i}(1:4)) = eul2quat(R0)';
    vals.x0(x_ind{i}(5:8)) = eul2quat(Rf)';
    vals.xf(x_ind{i}(1:4)) = eul2quat((vals.N-i)/(vals.N-1)*R0+(i-1)/(vals.N-1)*Rf)';
    vals.xf(x_ind{i}(5:8)) = eul2quat(Rf)';
    
    if i < vals.N
        vals.x0(t_ind{i}) = vals.dt;
        vals.xf(t_ind{i}) = vals.dt;
    end
end
...
plot_states(Z,x_ind,u_ind,t_ind,vals);
...
for i = 1:vals.N
    ...
    f = f + q_b_cost + q_t_cost + vel_cost + u_cost + t_cost;
end
...
[x_ind, u_ind, t_ind, x_ind_r, u_ind_r, t_ind_r] = get_inds(vals);
...
ceq(n_r*N+1:n_r*N+n_r) = state_error(Z(x_ind{end}), vals.xf(x_ind{end}));
...
function [x_ind, u_ind, t_ind, x_ind_r, u_ind_r, t_ind_r] = get_inds(vals)
...
for i = 1:N-1
    ...
    x_ind_r{i} = 1+(i-1)*(n_r+m+1):n_r+(i-1)*(n_r+m+1);
    u_ind_r{i} = n_r+1+(i-1)*(n_r+m+1):n_r+m+(i-1)*(n_r+m+1);
    t_ind_r{i} = n_r+m+1+(i-1)*(n_r+m+1);
end
x_ind{N} = 1+(N-1)*(n+m+1):n+(N-1)*(n+m+1);
x_ind_r{N} = 1+(N-1)*(n_r+m+1):n_r+(N-1)*(n_r+m+1);
end
