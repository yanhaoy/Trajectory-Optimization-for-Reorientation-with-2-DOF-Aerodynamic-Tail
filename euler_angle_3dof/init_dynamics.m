...
R_bt = [cos(q(5))*cos(q(6)), -cos(q(5))*sin(q(6)), sin(q(5));
    cos(q(4))*sin(q(6)) + cos(q(6))*sin(q(4))*sin(q(5)), cos(q(4))*cos(q(6)) - sin(q(4))*sin(q(5))*sin(q(6)), -cos(q(5))*sin(q(4));
    sin(q(4))*sin(q(6)) - cos(q(4))*cos(q(6))*sin(q(5)), cos(q(6))*sin(q(4)) + cos(q(4))*sin(q(5))*sin(q(6)),  cos(q(4))*cos(q(5))];
...
w_bt__b = skew2angvel(R_bt'*reshape(jacobian(reshape(R_bt, 9, 1), q(4:6))*q_dot(4:6), 3, 3));

% Jacobian matrix of tail angular velocity
J_bt__b = jacobian(w_bt__b, q_dot(4:6));

% Complete Jacobian
J__b = blkdiag(J_Ob__b, J_bt__b);

% Jacobian derivative
J_dot__b = reshape(jacobian(reshape(J__b, 36, 1), q)*q_dot, 6, 6);
...
% input should be same as roll, pitch and yaw
u = SX.sym('u', [6, 1]);
