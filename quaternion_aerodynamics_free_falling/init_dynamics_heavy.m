function [model] = init_dynamics_heavy()
...
% body; quaternions from body to tail; body center of mass position
q = SX.sym('q', [11, 1]);
...
% integrated speed should tend to zero. In addition, the tail's design is
% directional, so the COM translational speed will not affect a lot. While
% we don't use is to compute aerodynamic effect, we have to include it in
% system states, since now the system momentum and angular momentum are not
% conserved anymore

% Tail velocity's tangent component
v_ad = R_Ot'*p_dot_Ot__s - dot(R_Ot'*p_dot_Ot__s, l_t)*l_t/(l_t'*l_t);

% We know the translation velocity is a linear function of the length
k_a = jacobian(v_ad, length);
k_b = v_ad - k_a*length;

% Aerodynamic force should be opposite to the tangent velocity
f_ad_direction = -v_ad/sqrt(v_ad'*v_ad + 1e-12);

% Aerodynamic force is the integral on the sail along the tail length
f_ad_magnitude = rho*C_D*shaft_w/2* ...
    (k_a'*k_a*(tail_l^3 - shaft_l^3)/3 + ...
    2*k_a'*k_b*(tail_l^2 - shaft_l^2)/2 + ...
    k_b'*k_b*(tail_l - shaft_l));
...
u_ad_magnitude = rho*C_D*shaft_w/2* ...
...
% Aerodynamic torque in tail quaternion coordinates
u_ad_t = 2*quat2L(q(5:8))'*u_ad;

% Aerodynamic torque in body quaternion coordinates
...
w_dot = dynamics_func('q', q, 'q_dot', q_dot, 'u', [zeros(4, 1); J_u*u; zeros(3, 1)], 'length', tail_l);
w_dot = w_dot.w_dot;
